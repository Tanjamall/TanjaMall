...
create or replace function private.urlencode(input text)
returns text
language plpgsql
immutable
strict
...
declare
  ...
  index_value integer;
begin
  for index_value in 0..length(bytes) - 1 loop
    byte_value := get_byte(bytes, index_value);

    if (
      byte_value between 48 and 57
      ...
      hex_value := upper(lpad(to_hex(byte_value), 2, '0'));
...
comment on function private.urlencode(text) is
  'URL-encodes UTF-8 text for WhatsApp click-to-chat messages.';

revoke all on function private.urlencode(text)
from public, anon, authenticated;

drop function if exists public.create_cod_order(text, text, text, text, text, text, jsonb);
...
declare
  ...
  normalized_city text := coalesce(nullif(btrim(city), ''), 'Tanger');
...
begin
  ...
  if not (normalized_city = any(settings_row.supported_cities)) then
    raise exception 'city is not supported' using errcode = '22023';
  end if;
  ...
  delivery_amount := case
    when normalized_city = 'Tanger'
      and (
        settings_row.free_delivery_threshold is null
        or subtotal_amount < settings_row.free_delivery_threshold
      )
      then settings_row.delivery_fee_tanger
    else 0
  end;
  ...
  returning id, whatsapp_confirmation_url into new_order_id, whatsapp_confirmation_url;
...
comment on function public.create_cod_order(text, text, text, text, text, text, jsonb) is
  'Secure public COD order creation RPC. Validates cart and calculates totals from database product prices.';
