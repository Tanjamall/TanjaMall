    ...
    <article className="product-card">
      <Link href={`/products/${product.slug}`} className="product-media" aria-label={product.name}>
        {product.main_image_url ? (
          <img src={product.main_image_url} alt={product.name} loading="lazy" decoding="async" />
        ) : (
      ...
      <div className="product-info">
        ...
        {canOrder ? (
          <Link className="buy-btn" href={`/products/${product.slug}#product-order-form`}>اطلب</Link>
        ) : (
          <button type="button" className="buy-btn" disabled>غير متوفر</button>
        )}
      </div>
    </article>
  );
